...
def build_config_overrides(config):
    """Return list of config overrides for MAFT-Plus training."""
    overrides = []
...
def main():
    run = wandb.init(project="MAFT-Plus")
    cfg = run.config
    ...
    cmd = [
        "python", "train_net.py",
        "--config-file", "./configs/semantic/train_lars_coco.yaml",
        "--num-gpus", "8", 
        "--wandb",
        "--tag", run_tag,
        "MODEL.WEIGHTS", "/data/mfreiberg/weights/maftplus/maftp_l_pano.pth",
    ]
