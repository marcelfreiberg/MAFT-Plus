...
from maft.utils import WandbWriter, CommonMetricPrinter, WriterStack
...
def build_writers(cfg, max_iter, wandb_enabled=False, wandb_resume=False):
    ...
    """
    writers = [
        CommonMetricPrinter(max_iter, run_name=getattr(cfg, 'RUN_NAME', 'maft-plus')),
    ...
    if wandb_enabled:
        project = "MAFT-Plus"
        run_name = getattr(cfg, 'RUN_NAME', f'maft-plus-{cfg.MODEL.META_ARCHITECTURE}')
        writers.append(
            WandbWriter(
                max_iter=max_iter,
                run_name=run_name,
...
def main(args):
    ...
    if args.eval_only:
        ...
        with EventStorage(start_iter=0) as storage:
            with WriterStack(logger=logging.getLogger("maft-plus"), writers=writers):
                ...
                if res and comm.is_main_process() and writers:
                    ...
                    for k, v in flattened_results.items():
                        try:
                            ...
                            logging.getLogger("maft-plus").warning(f"Could not convert metric {k}={v} to float: {e}")
